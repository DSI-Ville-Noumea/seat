...
*/
public class OePMBPC extends nc.mairie.technique.BasicProcess {
...
//	 quand appuie sur entrée
	...
	if (null!=(getPMatInfosCourant())){
		if(null!=getPMatInfosCourant().getPminv()){
			...
			PM_Affectation_Sce_Infos unASI = PM_Affectation_Sce_Infos.chercherPM_Affectation_Sce_InfosCourantPm(getTransaction(),getPMatInfosCourant().getPminv(),date);
			if(getTransaction().isErreur()){
		...
		//	Si liste des BPC est vide
			...
			addZone(getNOM_ST_CARBU(),getPMatInfosCourant().getDesignationcarbu());
		}
	}else{
		setLB_BPC(LBVide);
 ...
 */
private void initialiseListeBPC(javax.servlet.http.HttpServletRequest request) throws Exception{
//	Recherche des bpc
	java.util.ArrayList a = BPC.listerBPCEquipement(getTransaction(),getPMatInfosCourant().getPminv());
	if (null == a){
