...
* 
	...
	public void initialiseZones(javax.servlet.http.HttpServletRequest request)
		...
		// EquipementInfos monEquipementInfos =
		// EquipementInfos.chercherEquipementInfos(getTransaction(),getOtCourant().getNuminv());
		EquipementInfos monEquipementInfos = new EquipementInfos();
		if (monOT != null) {
			if (monOT.getNuminv() != null) {
				monEquipementInfos = EquipementInfos.chercherEquipementInfos(
						getTransaction(), monOT.getNuminv());
			} else {
				monEquipementInfos = (EquipementInfos) VariableGlobale
						.recuperer(request, "EQUIPEMENTINFOS");
			}
		} else {
			monEquipementInfos = (EquipementInfos) VariableGlobale.recuperer(
					request, "EQUIPEMENTINFOS");
		}
		if (getTransaction().isErreur()) {
			return;
		}
		...
		/*
		 * EquipementInfos monEquipementInfos =
		 * (EquipementInfos)VariableGlobale.
		 * recuperer(request,"EQUIPEMENTINFOS"); if(monEquipementInfos!=null){
		 * setEquipementInfosCourant(monEquipementInfos); }
		 */
		...
		if (getOtCourant() != null) {
			...
			if (getEquipementInfosCourant() != null) {
				addZone(getNOM_ST_MARQUE(), getEquipementInfosCourant()
						.getDesignationmarque());
				addZone(getNOM_ST_MODELE(), getEquipementInfosCourant()
						.getDesignationmodele());
				addZone(getNOM_ST_TYPE(), getEquipementInfosCourant()
						.getDesignationtypeequip());
				addZone(getNOM_ST_NOINVENT(), getEquipementInfosCourant()
						.getNumeroinventaire());
				addZone(getNOM_ST_NOIMMAT(), getEquipementInfosCourant()
						.getNumeroimmatriculation());
				addZone(getNOM_ST_TCOMPTEUR(), getEquipementInfosCourant()
						.getDesignationcompteur());
				// infos concernant l'Ot
				addZone(getNOM_ST_NOOT(), getOtCourant().getNumeroot());
				addZone(getNOM_EF_COMMENTAIRE(), getOtCourant()
						.getCommentaire());
				...
				ArrayList a = Planning.chercherPlanningOt(getTransaction(),
						getOtCourant().getNumeroot());
				if (getTransaction().isErreur()) {
					return;
				}
	...
	public void initialiseListEntretiens(
			javax.servlet.http.HttpServletRequest request, ArrayList a)
			throws Exception {
		int indice = 0;
		int i = 0;
		setListEntretien(a);
...
* 
	...
	public boolean performPB_IMPRIMER(
		...
		if (!newCommentaire.equals("")) {
			if (newCommentaire.length() > 600) {
				getTransaction()
						.declarerErreur("Le commentaire est trop long.");
				return false;
			}
			// si le commentaire a été modifié
			if (!newCommentaire.equalsIgnoreCase(getOtCourant()
					.getCommentaire())) {
				getOtCourant().setCommentaire(newCommentaire);

				// recherche de l'équipement a passé en paramètre
				if (getOtCourant().getNuminv() == null) {
					unEquipement = Equipement.chercherEquipement(
							getTransaction(), getEquipementInfosCourant()
									.getNumeroinventaire());
				} else {
					unEquipement = Equipement.chercherEquipement(
							getTransaction(), getOtCourant().getNuminv());
				}
				if (getTransaction().isErreur()) {
					return false;
				}
		...
		if (getOtCourant() != null) {
			...
			String nomequip = getZone(getNOM_ST_MARQUE()) + " "
					+ getZone(getNOM_ST_MODELE());
			...
			// pour les commentaires avec retour à la ligne par la touche
			// "Entrée"
			...
			for (int i = 0; i < comlen; i++) {
				if (commentaire.charAt(i) == '\n') {
					commentaireOt = commentaireOt
							+ commentaire.substring(indice, i) + "$";
					indice = i;
				}
			}
			commentaireOt = commentaireOt
					+ commentaire.substring(indice, comlen);
			...
			for (int i = 0; i < getListEntretien().size(); i++) {
				if (!getListEntretien().get(i).equals("")) {
					...
					out.write("2");
					out.write(StringUtils.leftPad(unplan.getLibelleentretien(), 31, " "));
					out.write(StringUtils.leftPad(unplan.getDatereal(), 11, " "));
					out.write(StringUtils.leftPad(unplan.getDuree(), 6, " "));
					out.write("\n");
				}
			}
			out.flush();
			...
			// StarjetGeneration g = new StarjetGeneration(getTransaction(),
			// "MAIRIE", starjetMode, "SEAT", "ficheOT_Vierge.sp",
			// "ficheOT_Vierge");
	...
	public String getServiceForEquipement(Equipement equipement,
			String DateEntree) {
		String sservice = "";
		try {
			AffectationServiceInfos unASI = AffectationServiceInfos
					.chercherAffectationServiceInfosCourantEquip(
							getTransaction(), getEquipementInfosCourant()
									.getNumeroinventaire(), Services
									.dateDuJour());
	...
	public boolean performPB_EQUIP(javax.servlet.http.HttpServletRequest request)
		...
		if (recherche.equals("")) {
			getTransaction().declarerErreur(
					"La zone de recherche doit être renseignée.");
			return false;
		}
		EquipementInfos unEquipementInfos = EquipementInfos
				.chercherEquipementInfosInvOuImmat(getTransaction(), recherche);
		if (getTransaction().isErreur()) {
			getTransaction().declarerErreur(
					"L'équipement recherché n'a pas été trouvé.");
			return false;
		}
		...
		VariableGlobale.ajouter(request, "EQUIPEMENTINFOS",
				getEquipementInfosCourant());
		return true;
	}
...
* 
	...
	public boolean performPB_COMMENTAIRE(
		...
		Equipement unEquipement = Equipement.chercherEquipement(
				getTransaction(), getEquipementInfosCourant()
						.getNumeroinventaire());
		if (getTransaction().isErreur()) {
