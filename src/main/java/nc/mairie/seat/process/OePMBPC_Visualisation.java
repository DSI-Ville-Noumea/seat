...
* 
*/
public class OePMBPC_Visualisation extends nc.mairie.technique.BasicProcess {
		...
		if(getBpcCourant().getNumerobpc()!=null){
			...
			if(getPMaterielCourant()!=null){
				if(getPMaterielCourant().getPminv()!=null){
					PM_Affectation_Sce_Infos unPMASI = PM_Affectation_Sce_Infos.chercherPM_Affectation_Sce_InfosCourantPm(getTransaction(),getPMaterielCourant().getPminv(),getBpcCourant().getDate());
					if(getTransaction().isErreur()){
						//si pas trouvé, c'est fonctionellement normal, on traite l'erreur
						getTransaction().traiterErreur();
						addZone(getNOM_ST_SERVICE(),"pas affecté");
...
public String getJSP() {
	return "OePMBPC_Visualisation.jsp";
}
/**
 * Retourne le nom d'un bouton pour la JSP :
 * PB_RECHERCHER
