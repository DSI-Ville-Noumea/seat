...
/**
 Methode à définir dans chaque objet Métier pour instancier un Broker 
*/
protected AgentCCASBroker getMyAgentCCASBroker() {
	return (AgentCCASBroker)getMyBasicBroker();
}
/**
...
//	 on ne garde que les agents actifs
	if (listeAgentCCAS.size()>0){
		for (int i=0;i<listeAgentCCAS.size();i++){
			AgentCCAS unAgent = (AgentCCAS)listeAgentCCAS.get(i);
			if(!unAgent.getNom().equals("")){
				AgentCCASSppost unSppost = AgentCCASSppost.chercherAgentCCASSppost(aTransaction,unAgent.getNomatr());
				if(aTransaction.isErreur()){
	...
	return listFinale;
	*/
	return listeAgentCCAS;
}
/**
 * Retourne un AgentCCAS.
 * @return AgentCCAS
 */
public static AgentCCAS chercherAgentCCAS(nc.mairie.technique.Transaction aTransaction, String code) throws Exception{
	AgentCCAS unAgentCCAS = new AgentCCAS();
	return unAgentCCAS.getMyAgentCCASBroker().chercherAgentCCAS(aTransaction, code);
}
/**
 * Retourne un AgentCCAS.
 * @return AgentCCAS
