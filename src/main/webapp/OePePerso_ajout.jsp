...
<jsp:useBean class="nc.mairie.seat.process.OePePerso_ajout" id="process" scope="session"></jsp:useBean>
					...
					<tr>
						<td colspan="3">
							...
							<TR>
								...
								<SELECT size="1" name="<%=process.getNOM_LB_ENTRETIEN() %>"
									...
									<%= process.forComboHTML(process.getVAL_LB_ENTRETIEN(),process.getVAL_LB_ENTRETIEN_SELECT()) %>
												...
												<TD><SELECT size="1"
									name="<%=process.getNOM_LB_TENT() %>" class="sigp2-liste" style="text-transform: uppercase" onchange='executeBouton("<%= process.getNOM_PB_OK_TENT()%>")'>
							...
							<TR>
								...
								<TD></TD>
								<TD><INPUT type="submit" name="<%=process.getNOM_PB_OK_TENT() %>"  style="visibility : hidden;" value="ok"></TD>
							</TR>
<%if (!process.getIsOcassionnel()){ %>
							<TR>
								<TD>Intervalle</TD>
