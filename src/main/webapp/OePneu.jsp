...
<TITLE>OePneus.jsp</TITLE>
</HEAD>
<jsp:useBean class="nc.mairie.seat.process.OePneu" id="process" scope="session"></jsp:useBean>
...
<%@ include file="BanniereErreur.jsp" %>
    <TR>
      <TD style="text-align : center;" width="681"><FORM name="formu" method="POST"><FIELDSET><LEGEND class="sigp2Fieldset" align="left"></LEGEND><TABLE border="0" cellspacing="0" cellpadding="0" class="sigp2">
				<TR>
					<TD height="26" width="300">
					<TABLE border="0" class="sigp2">
						<TR align="center">
							<TD>La liste des dimensions</TD>
						</TR>
					</TABLE>
					</TD>
				</TR>
				<TR>
					<TD>
						<SELECT size="15" name="<%= process.getNOM_LB_LIST_DIMENSION() %>"
					class="sigp2-liste" style="text-transform: uppercase; width: 100%">
					<%= process.forComboHTML(process.getVAL_LB_LIST_DIMENSION(),process.getVAL_LB_LIST_DIMENSION_SELECT()) %>
					...
					<TABLE border="0" class="sigp2">
						<TR align="center">
							...
							<TD class="sigp2-saisie" style="text-transform: uppercase"><%if ("".equals(process.getVAL_ST_DESIGNATION())){ %>
								<INPUT type="text" size="40" name="<%= process.getNOM_EF_LIBELLE_DIMENSION() %>"
								class="sigp2-saisie" value="<%= process.getVAL_EF_LIBELLE_DIMENSION()%>" maxlength="30" style="text-transform: uppercase">
								<%}else{ %>
