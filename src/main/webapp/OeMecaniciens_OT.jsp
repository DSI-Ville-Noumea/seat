...
<jsp:useBean class="nc.mairie.seat.process.OeMecaniciens_OT" id="process" scope="session"></jsp:useBean>
...
<%@ include file="BanniereErreur.jsp" %>
    <TR>
      <TD style="text-align : center;">
			<FORM name="formu" method="POST"><SPAN class="sigp2-titre"><%=process.getVAL_ST_TITRE_ACTION() %>d'un mécanicien pour l'OT <%=process.getVAL_ST_NOOT() %><BR>
		<BR></SPAN>
...
<%if(process.tailleListeMecaOT){ %>
				<INPUT
					type="submit" value="&lt;&lt;Enlever"
					name="<%=process.getNOM_PB_ENLEVER() %>" class="sigp2-Bouton-100"></TD>
<%} %>
					<TD><!-- Mécaniciens intervenant sur l'OT -->
						<FIELDSET><LEGEND class="sigp2Fieldset" align="left">Mécaniciens pour l'OT</LEGEND><TABLE border="0" class="sigp2">
							<TR align="center">
								<TD align="left">
							...
							<TR>
								<TD align="left">
									<SELECT size="5" name="<%= process.getNOM_LB_MECA_OT() %>"
								class="sigp2-liste" style="text-transform: uppercase; width: 100%" ondblclick='executeBouton("<%= process.getNOM_PB_ENLEVER()%>")'>
								<%= process.forComboHTML(process.getVAL_LB_MECA_OT(),process.getVAL_LB_MECA_OT_SELECT()) %>
							</SELECT>
							</TR>
							<TR>
