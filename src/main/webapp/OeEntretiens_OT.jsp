...
<%@ include file="BanniereErreur.jsp" %>
    <TR>
      <TD style="text-align : center;">
			<FORM name="formu" method="POST"><SPAN class="sigp2-titre"><%=process.getVAL_ST_TITRE_ACTION() %> d'un entretien pour l'OT <%=process.getVAL_ST_NOOT() %><BR></SPAN>
				...
				<TABLE class="sigp2">
					...
					<TR>
						...
						<TD><INPUT type="submit" name="<%=process.getNOM_PB_OK_TENT() %>"
							style="visibility : hidden;" value="ok"></TD>
						<TD></TD>
					</TR>
					<TR>
						<TD>Prévu le</TD>
					...
					<%if (!process.isOcassionnel()){ %>
					<TR>
						<TD>Intervalle</TD>
						<TD><INPUT type="text" size="5"
							name="<%=process.getNOM_EF_INTERVALLE() %>"
							value="<%=process.getVAL_EF_INTERVALLE() %>" class="sigp2-saisie"
							style="text-transform: uppercase"></TD>
						<TD></TD>
						<TD>Type d'intervalle</TD>
						<TD><SELECT size="1" name="<%=process.getNOM_LB_TINT() %>"
							class="sigp2-liste" style="text-transform: uppercase">
							<%= process.forComboHTML(process.getVAL_LB_TINT(),process.getVAL_LB_TINT_SELECT()) %>
						</SELECT></TD>
						<TD></TD>
					</TR>
					<TR>
						<TD>Sinistre</TD>
						<TD><INPUT type="checkbox" <%= process.forCheckBoxHTML(process.getNOM_CK_SINISTRE() , process.getVAL_CK_SINISTRE()) %> ></TD>
						<TD></TD>
						<TD></TD>
						<TD></TD>
						<TD></TD>
